...
class TestWrapperSB3(unittest.TestCase):
    """
    Unit tests for module math.py.
    """

## -------------------------------------------------------------------------------------------------
    def test_wrapper_sb3(self): 
        env_cls     = A2C
        buffer_size = 5
        ...
        self.assertGreater(len(cus_callback.loss_cnt),0, "No Loss on Native")
        self.assertGreater(len(training.get_scenario().policy_wrapped.loss_cnt),0, "No Loss on Wrapper")
        length = min(len(cus_callback.loss_cnt), len(training.get_scenario().policy_wrapped.loss_cnt))
        self.assertEqual( np.linalg.norm(np.array(cus_callback.loss_cnt[:length])-np.array(training.get_scenario().policy_wrapped.loss_cnt[:length])), 0.0 , "Mismatch Native and Wrapper")
