...
class StreamMLProDynamicClouds2D (StreamMLProBase):
    """
    This demo stream provides 250 2-dimensional instances per cluster randomly positioned around 
    ...
    def _init_dataset(self):
        ...
        for x in range(self.no_clouds):
            ...
            if mag != 0:
                ...
                final_centers[x][:] = centers[x][:] + (0.5**0.5)*self.C_NUM_INST_PER_CLOUD*self.velocity
        ...
        if self.pattern == 'merge':
            ...
            for x in range(self.no_clouds-m):
                ...
                if mag != 0:
                    ...
                    centers[m+x][:] = final_centers[m+x][:] + (0.5**0.5)*self.C_NUM_INST_PER_CLOUD*self.velocity


        # 2 Create self.C_NUM_INSTANCES noisy inputs around each of the 4 hotspots
